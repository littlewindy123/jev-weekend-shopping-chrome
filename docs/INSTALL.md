...
页面出现 **双休购物 0.4.4** 卡片，即已加载成功。不要选择 ZIP 文件，也不要选择外面多套的一层空文件夹。
...
图中是 0.4.3 尚未配置的初始面板，0.4.4 操作相同，故显示「已暂停」。连接测试会发送一个虚构商品请求；不要把自己的 Key 发到 Issue 或公开截图里。
...
左边的 PASS 是插件加上的；相邻商品保持原样。盖章仍可点击商品，图片下方保留「AI 猜测 · 未核实」提示。

淘宝也是直接在原商品图上盖章，下面是用户提供的 0.4.4 实际运行截图：
