...
解压后可能还有一层同名文件夹；继续打开，直到看到下面这些文件：

```text
jev-weekend-shopping-chrome-main/  ← 选择这一层文件夹
├── manifest.json
├── background.js
├── popup.html
└── icons/
...
文件夹名字可能带 `(1)`、`(2)`，不影响安装。**关键是直接包含 `manifest.json`，选整个文件夹，不是选这个 JSON 文件。**
...
页面出现 **双休购物 0.4.6** 卡片，即已加载成功。不要选择 ZIP 文件，也不要选择外面多套的一层空文件夹。

![Chrome 扩展管理页：左上红框标出加载未打包的扩展程序，中间红框为已安装的双休购物 0.4.6](chrome-install-0.4.6.png)

**照着红框操作：左上角是安装入口，中间是安装成功的样子。** 深绿购物袋是新版图标。「打包扩展程序」不是安装按钮；第一次安装也不用点「更新」。
...
图中是 0.4.3 尚未配置的初始面板，0.4.6 操作相同，故显示「已暂停」。连接测试会发送一个虚构商品请求；不要把自己的 Key 发到 Issue 或公开截图里。
